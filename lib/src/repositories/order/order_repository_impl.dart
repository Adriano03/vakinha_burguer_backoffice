...
class OrderRepositoryImpl implements OrderRepository {
  ...
  Future<void> changeStatus(int id, OrderStatus status) async {
    try {
      await _dio.auth().put(
        '/orders/$id',
        data: {
          'status': status.acronyn,
        },
      );
    } on DioError catch (e, s) {
      log('Erro ao alterar status do pedido para ${status.acronyn}', error: e, stackTrace: s);
      throw RepositoryException(
        message: 'Erro ao alterar status do pedido para ${status.acronyn}',
      );
    }
  }
    ...
    try {
      final orderResponse = await _dio.auth().get(
        ...
        queryParameters: {
          ...
          if (status != null) 'status': status.acronyn,
        },
      );
  ...
  Future<OrderModel> getById(int id) async {
    try {
      ...
      log('Erro ao buscar o pedidos $id', error: e, stackTrace: s);
      throw RepositoryException(
        message: 'Erro ao buscar o pedidos $id',
      );
    }
  }
}
